/-!
# Source package model for `jacobian-conjecture-canonical-lane`
...
def sourceCheckoutHead : String := "cd02421f280def96fe26348f2e05a7bd087c7438"
...
def sourceFiles : List SourceFile := [
  ...
  { path := "scripts/jc_closure_guard.py", sha256 := "1f305b01ecde2d3c2058314c2d1dbbc1dec0150e44b064d66be7c9fb70661255", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  ...
  { file := "scripts/jc_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/jc_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/jc_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/jc_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 58, isAsync := false },
  { file := "scripts/jc_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 70, isAsync := false },
  { file := "scripts/jc_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 87, isAsync := false },
  { file := "scripts/jc_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 98, isAsync := false },
  { file := "scripts/jc_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 187, isAsync := false },
  { file := "scripts/jc_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 199, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  ...
  { file := "scripts/jc_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/jc_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/jc_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/jc_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/jc_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/jc_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/jc_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_jacobian\", \"sigma_degree\", \"kappa_compact\", \"rho_rigidity\", \"inverse_transfer\"]", line := 21 },
  { file := "scripts/jc_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_jacobian", status := "derived_numeric", formula := "c_jac_raw * jac_density_raw - e_jac_raw", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["c_jac_raw", "e_jac_raw", "jac_density_raw"] },
  { group := "constants", key := "sigma_degree", status := "derived_numeric", formula := "degree_floor_raw - reduction_loss_raw - restart_loss_raw", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["degree_floor_raw", "reduction_loss_raw", "restart_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "inverse_transfer", status := "derived_numeric", formula := "c_inv_raw * transfer_gain_raw - e_inv_raw", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_inv_raw", "e_inv_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Appendix E.6" },
  { key := "inverse_transfer", value := "1.029658", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#inverse_transfer", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "kappa_compact", value := "0.8025682182985554", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "kappa_jacobian", value := "1.09644", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_jacobian", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "rho_rigidity", value := "1.072", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "sigma_degree", value := "1.067", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_degree", sourceSection := "paper/JACOBIAN_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B" }
]
